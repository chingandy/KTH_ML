...
for i in range(10):
    for j in range(10):
        v = numpy.array(data[i][0:2])- numpy.array(data[j][0:2])
        P[i][j]= ( math.exp(-(numpy.inner(v,v)/(2*2**2))) )*data[i][2]*data[j][2]
...
def indicator(x, y):
    ...
    for i in range(len(xi)):
        ...
        sum += alpin[i]*xi[i][2]*( math.exp(-(numpy.inner(v,v)/(2*2**2))) )
    return sum
