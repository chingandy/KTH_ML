...
def computePrior(labels, W=None):
    ...
    for i in range(Nclasses):
        wh = np.where(labels == classes[i])
        prior[i] = np.size(wh)/Npts
    # ==========================
...
def classifyBayes(X, prior, mu, sigma):
    ...
    for i in range(Nclasses):
        for j in range(Npts):
             ...
             logProb[i][j] = -0.5*np.log(np.linalg.det(sigma[i])) -0.5*(np.dot(np.dot(xmu, np.linalg.inv(sigma[i])) ,xmu))+ np.log(prior[i])
